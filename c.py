...
# img, gray_blur = read_img('OpenCV/test_image/2D/2D_gray_test.jpg')
img, gray_blur = read_img('OpenCV/test_image/2D/test222.jpg')
# print(img)
# cv2.imshow('img', img)
# cv2.waitKey()
...
edges = canny_edge(gray_blur, sigma=0.33)
...
lines = hough_line(edges, min_line_length=100, max_line_gap=10)
...
def write_crop_images(img, points, img_count=0, folder_path='OpenCV/test_image/2D'):
    num_list = []
    ...
    for row in num_list:
        ...
        for s in row:
            ...
            cv2.imwrite('OpenCV/test_image/2D/test/' + str(img_count) + '.jpg', cropped)
            # cv2.imwrite(folder_path + 'data' + str(img_count) + '.jpg', cropped )

            break  # 생략시 오류 , 존재할 경우 8가지만 생성 
        # if row[0] == 8:
            # break
...
img_count = write_crop_images(img, points, img_count=0, folder_path='OpenCV/test_image/2D/test/')

# cv2.imshow('write_crop_images', img_count)
# cv2.imwrite('OpenCV/test_image/2D/test.jpg', img_count)
cv2.waitKey()
...
# img_list = x_crop_images(img, points)
...
# img_data = convert_image_to_bgr_numpy_array(image_path, size=(224, 224))
