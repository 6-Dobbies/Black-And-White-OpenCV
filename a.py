...
def write_crop_images(img, points, img_count, folder_path='OpenCV/test_image/2D'):
    num_list = []
    ...
    for row in num_list:
        for s in row:
            ...
            print("+++ ", './test_image/2D/test' + str(img_count) + '.jpeg', cropped)

            # [] 빈배열의 문제로 에러가 발생할시 다음것으로 넘어가는게 아니라 for문 으로 다시 돌아가서 일해라! 
            if len(cropped) == 0:
                continue

            cv2.imwrite('OpenCV/test_image/2D/test/' + str(img_count) + '.jpg', cropped)
            # cv2.imwrite('./test_image/2D/test' + str(img_count) + '.jpeg', cropped)
...
img_filename_list = []
folder_name = 'OpenCV/test_image/2D/c6.jfif'
...
for file_name in img_filename_list:
    ...
    # 이미지에따라 값이 다름, 출력된 값을 확인하여 assert 의 비교값에 입력
    print(len(h_lines))
    print(len(v_lines))

    assert len(h_lines) >= 15
